...
class collectData3 {
   ...
   function getAllCompaniesCouchCpa($DbPath,$lucenePath,$Db,$DesignDoc,$Index,$Wc,$Limit,$Sort,$varKeyword,$couchUser,$couchPass,$Url,$term,$orgtypescouchDB,$chamberscouchDB,$advCriteria){
       $results =     $this->prepareResultsCouchCpa($DbPath,$lucenePath,$Db,$DesignDoc,$Index,$Wc,$Limit,$Sort,$varKeyword,$couchUser,$couchPass,$Url,$term,$orgtypescouchDB,$chamberscouchDB,$advCriteria);   
       $rows =  $results[0];
       $totals =  $results[1];
       return [$rows,$totals];     		  
   }
   ...
   function getAllCompaniesCouchEle($DbPath,$lucenePath,$Db,$DesignDoc,$Index,$Wc,$Limit,$Sort,$varKeyword,$couchUser,$couchPass,$Url,$term,$orgtypescouchDB,$chamberscouchDB,$advCriteria){
       
       return $this->prepareResultsCouchEle($DbPath,$lucenePath,$Db,$DesignDoc,$Index,$Wc,$Limit,$Sort,$varKeyword,$couchUser,$couchPass,$Url,$term,$orgtypescouchDB,$chamberscouchDB,$advCriteria);       		  
   }
   ...
   function prepareResultsCouchCpa($DbPath,$lucenePath,$Db,$DesignDoc,$Index,$Wc,$Limit,$Sort,$varKeyword,$couchUser,$couchPass,$lbUrl,$term,$orgtypescouchDB,$chamberscouchDB,$advCriteria){
       $couchUserPwd = $couchUser.':'.$couchPass;
       $blacklist=['Ο ΑΦΜ'];
       $ch = curl_init();
       if ($varKeyword ==='cpa' || $varKeyword ==='CPA' || varKeyword ==='' ){
           $simpleSearch = '';
       }
       else {
            $simpleSearch = $term.":".$varKeyword;
       }
       $url = $DbPath.$lucenePath.$Db."/_design/".$DesignDoc."/".$Index."?q=".$simpleSearch.$Wc.$advCriteria."&limit:".$Limit."&sort:".$Sort;
               ...
               global $Boost;
                ...
                if  ($arrayElements <= 1000 && isset($newdata)){
                    #echo $arrayElements;
                      ...
                      if ($key === NULL && strpos($newdata['vat'], $blacklist[0])===false){
   ...
   function prepareResultsCouchEle($DbPath,$lucenePath,$Db,$DesignDoc,$Index,$Wc,$Limit,$Sort,$varKeyword,$couchUser,$couchPass,$lbUrl,$term,$orgtypescouchDB,$chamberscouchDB,$advCriteria){
       $couchUserPwd = $couchUser.':'.$couchPass;
       $ch = curl_init();
